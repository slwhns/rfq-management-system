...
<div id="quotes-page-staff" class="d-grid gap-20" style="grid-template-columns: 1fr;" data-status-update-url-template="{{ route('quotes.status.update', ['id' => '__QUOTE_ID__']) }}" data-response-update-url-template="{{ route('quotes.staff-response.update', ['id' => '__QUOTE_ID__']) }}">
    <div class="bg-white5 pd-20 br-10 box-shadow-basic">
        <div class="d-flex jc-between ai-center mg-b-15">
            ...
            <div class="d-flex ai-center" style="gap: 10px;">
                <form id="quote-filter-form" method="GET" action="{{ route('quotes.index') }}" class="d-flex ai-center" style="gap: 8px;">
                            ...
                            <tr style="border-bottom:1px solid #ececec;">
                                ...
                                <td style="padding:10px 8px; text-align:center;">
                                    ...
                                    <div class="d-flex ai-center jc-center" style="gap:8px; flex-wrap:wrap;">
                                        <select class="quote-status-select pd-5 bdr-all-22 br-5 fs-12" data-quote-id="{{ $quote->id }}" data-prev-status="{{ $normalizedStatus }}" style="border: 1px solid {{ $isApprovedStatus ? '#bdeacb' : '#d9d9d9' }}; background: {{ $isApprovedStatus ? '#eafaf0' : '#ffffff' }}; color: {{ $isApprovedStatus ? '#1f8a4c' : '#222222' }}; min-width: 132px;" {{ ($isDeclinedStatus || $isApprovedStatus) ? 'disabled' : '' }}>
                                            @if(!$canSelectCurrent)
                                            ...
                                            @foreach($statusUpdateOptions as $status => $optionLabel)
                                                @if($status === \App\Models\Quote::STATUS_DECLINED)
                                                    @continue
                                                @endif
                                ...
                                <td style="padding:10px 8px;">
                                    <div class="d-flex" style="flex-direction:column; gap:6px;">
                                        <div class="mg-b-4 pd-8 br-5" style="background:#f8f9fb; border:1px solid #e2e6ef;">
                                            <div class="fs-11 fw-bold clr-grey1 mg-b-4">Admin Note</div>
                                            @if(!empty($quote->admin_notes))
                                                <div class="fs-12" style="white-space:pre-wrap; word-break:break-word;">{{ $quote->admin_notes }}</div>
                                                @if($quote->admin_notes_updated_at)
                                                    <div class="fs-11 clr-grey1 mg-t-5">Comment by {{ $quote->adminNotesUpdatedBy->name ?? 'Admin' }} - {{ optional($quote->admin_notes_updated_at)->format('d M Y, h:i A') }}</div>
                                                @endif
                                            @else
                                                <div class="fs-11 clr-grey1">No note from admin</div>
                                            @endif
                                        </div>
                                        <div class="fs-11 fw-bold clr-grey1">Response</div>
                                        <textarea
                                            class="quote-staff-response pd-8 bdr-all-22 br-5 fs-12"
                                            data-quote-id="{{ $quote->id }}"
                                            style="width:100%; min-height:72px; resize:vertical; font-family:inherit;"
                                            placeholder="Example: Done updated quantity to 12.">{{ $quote->staff_response }}</textarea>
                                        <div class="d-flex ai-center" style="gap:8px;">
                                            <button type="button" class="save-staff-response-btn pd-6 br-5 fs-11 cursor-pointer" data-quote-id="{{ $quote->id }}" style="border:1px solid #1f8a4c; background:#1f8a4c; color:#fff;">Save Response</button>
                                            <span class="staff-response-status fs-11 clr-grey1" data-quote-id="{{ $quote->id }}"></span>
                                        </div>
                                        @if($quote->staff_response_updated_at)
                                            <div class="fs-11 clr-grey1">Response by {{ $quote->staffResponseUpdatedBy->name ?? 'Staff' }} - {{ optional($quote->staff_response_updated_at)->format('d M Y, h:i A') }}</div>
                                        @endif
                                    </div>
                                </td>
                                <td style="padding:10px 8px; text-align:center;">
